...
async def initialize_data():
    ...
    for i, store_data in enumerate(TOTTUS_STORES):  # Use real Tottus stores
        ...
        store = Store(
            ...
            address=store_data.get("address", f"Dirección {i+1}, {store_data.get('comuna', 'Santiago')}"),
            latitude=store_data["lat"] + random.uniform(-0.001, 0.001),
            longitude=store_data["lon"] + random.uniform(-0.001, 0.001),
            status=status,
